...
# - Modified to work correctly with May 2019 data release
#   (column names don't match 2018 release) 
# Created by Jenna Daly
# Modified by Ilya Ilyankou
# On 06/03/2019
#
##################################################################
...
drug_type_df <- within(drug_type_df, Ethanol[grep("Ethanol", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
...
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Buprenorphine", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("opioid", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("meperidine", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("opiate", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("opiate", drug_type_df$Other, ignore.case=T)] <- "Y")

# Any rows that list morphine in either cause of death or `Other` columns will have "Any Opioid" marked as "Y"
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("morphine", drug_type_df$ImmediateCauseA, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("morphine", drug_type_df$Other, ignore.case=T)] <- "Y")

# Any row where any of the following are marked - set "Any Opioid" as "Y"
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Heroin, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Fentanyl, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Oxycodone, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Oxymorphone, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Hydrocodone, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Methadone, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$Tramad, ignore.case=T)] <- "Y")
drug_type_df <- within(drug_type_df, `AnyOpioid`[grep("Y", drug_type_df$`Morphine_NotHeroin`, ignore.case=T)] <- "Y")

# Rename geo column to Town, sex column to gender
drug_type_df <- drug_type_df %>% 
  rename(Town = DeathCity, Gender = Sex, `Any Opioid`=AnyOpioid)
  
# Trim cases that we cannot compute
drug_type_df <- drug_type_df[!(drug_type_df$Gender %in% c('', 'Unknown')) & (drug_type_df$Date != ""),] #(remove 4 cases)

table(drug_type_df['Gender'])

# Pull years out of date stamps
drug_type_df$Year <- gsub("([0-9]{2})/([0-9]{2})/([0-9]{4}) ([0-9]{2}):([0-9]{2}):([0-9]{2}) ([A-Z]{2})", "\\3", drug_type_df$Date)
...
drug_type_df <- drug_type_df %>% 
  mutate(Town = ifelse((drug_type_df$Town == "") & (drug_type_df$Location == "Residence") & (drug_type_df$`ResidenceCity` != ""), `ResidenceCity`, Town))
...
raw[
  ,
  listedDrugs := paste(
    Heroin, Cocaine, Fentanyl, Oxycodone, Oxymorphone, Ethanol, Hydrocodone, Benzodiazepine, 
    Methadone, Amphet, Tramad, `Morphine_NotHeroin`, sep = ""),
  by = list(Heroin, Cocaine, Fentanyl, Oxycodone, Oxymorphone, Ethanol, Hydrocodone, Benzodiazepine, 
            Methadone, Amphet, Tramad, `Morphine_NotHeroin`)
  ]
...
write.table(
  drug_data,
  file.path(getwd(), "data", "accidental-drug-related-deaths-by-drug-type_2012-2018.csv"),
  sep = ",",
  row.names = F,
  na = "-9999"
)
